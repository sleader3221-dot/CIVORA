      ...
      <aside className="sidebar" aria-label="Primary navigation">
        ...
        <div className="sidebar__bottom">
          ...
          <div style={{ position: "relative" }}>
            <button type="button" className="profile" onClick={() => setProfileMenu((current) => !current)}>
              <span className="profile__avatar">{(user?.user ?? "NR").split(" ").map((p) => p[0]).slice(0, 2).join("")}</span>
              <span>
                <strong>{user?.user ?? "Nadia Rahman"}</strong>
                <small>{user?.role ?? "Site Safety Lead"}</small>
              </span>
              <ChevronDown size={14} />
            </button>
            {profileMenu && (
              <div className="site-menu" style={{ position: "absolute", bottom: "100%", left: 0, right: 0, marginBottom: 4, zIndex: 60 }}>
                <button type="button" onClick={() => { setProfileMenu(false); onLogout(); }}>
                  <span>Sign out</span>
      ...
      <div className="app-body">
        <header className="topbar">
          ...
          <div className="topbar__actions">
            ...
            <div className="sync-state" style={{ marginRight: 6 }}>
              <span>{liveClock.toLocaleTimeString()}</span>
            </div>
            <button
              type="button"
